...
# Função para carregar o top_score
def load_top_score():
    file_path = 'top_score.json'
    if os.path.exists(file_path):
        with open(file_path, 'r') as file:
            data = json.load(file)
            return data.get('top_score', 0)
    else:
        # Se o arquivo não existe, cria um com o top_score inicial
        with open(file_path, 'w') as file:
            json.dump({'top_score': 0}, file)
        return 0

# Função para salvar o top_score no arquivo JSON
def save_top_score(new_top_score):
    file_path = 'top_score.json'
    with open(file_path, 'w') as file:
        json.dump({'top_score': new_top_score}, file)
...
class Game(object):
    ...
    def run(self):
        ...
        top_score = load_top_score()  # Carrega o top_score salvo no arquivo JSON
        ...
        while(True):
            ...
            while (True):
                ...
                if energy <= 0 and press_keys == True:
                    if points > top_score:
                        top_score = points
                        save_top_score(top_score)  # Salva o novo top_score
